    ...
    public sealed class ChemicalFormula : IEquatable<ChemicalFormula>
    {
        ...
        public static ChemicalFormula ParseFormula(string formula)
        {
            ...
            if (!ValidateFormulaRegex.IsMatch(formula))
                throw new TestLibException("Input string for chemical formula was in an incorrect format: " + formula);
        ...
        {
            if (Elements.Count > 0)
                throw new TestLibException("Cannot know for sure what the number of neutrons is!");
